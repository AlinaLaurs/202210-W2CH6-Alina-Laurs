...
//Más variables que se van a utilizar.
let nameHTML;
let playButton;
let answerHTML;
let questionHTML;
...
//Todas las letras del HTML se guardan en circles.
const circles = document.querySelectorAll('.letter');
//Contenedor principal.
const container = document.getElementById('main-container');
//Número de letras.
const n = 27;
//Logitud del radio del círculo.
const r = 300;
//Formula que permite colocar la A (primera letra) arriba en el centro según el ángulo.
let angulo = -Math.PI / 2;
//Aquí se define el origen (las coordenadas) del primer círculo.
...
//Cada 20 milisegundos se va pintando cada uno de los círculos de las letras, en el cículo (aumentando el ángulo).
//La formula del círculo fue encontrada en Internet.
setInterval(() => {
...
//Es la función principal, la función de ejecución donde aparecen todos los elementos HTML enlazados.
const main = () => {
    //Los contenedores 2 y 3 no se verán hasta cambiar su estado.
    ...
    //Escuchadores para ejecutar funcionnes.
...
//Se ejecutará al presionar el botón de finalizar.
const endGame = () => {
    isPlaying = false;
    //Se visualiza el tercer bloque (y se esconde el segundo).
    document.getElementById('two').style.display = 'none';
    document.getElementById('two').style.visibility = 'hidden';
    document.getElementById('three').style.display = 'block';
    document.getElementById('three').style.visibility = 'visible';
    //Quito la sombra de la letra.
...
const getName = () => {
    ...
    if (userName === null || userName.trim().length === 0) {
        ...
        //Tras introducir el nombre, se inicilizará el juego.
        startGame();
    }
};

//Llamar getName cuando presionas enter en la primera pantalla.
...
const startGame = () => {
    randomGame();
    isPlaying = true;
    //Se asigna la pregunta número 0 a la caja de questionHTML (y se muestra en el bloque 2).
    questionHTML.textContent = questions[currentPosition].question;
    //Se asigna la somra a la letra A.
    lettersHTML[0].style.boxShadow = '0 0 15px 3px #fff';
    //Se pone el nombre del jugador.
...
const updateTimer = () => {
    ...
    //Gracias a esto, se va restando cada segundo a partir de 150 hasta llegar a 0.
    else if (isPlaying) {
        seconds--;
...
//Aquí se genera un rosco random cogiendo preguntas de los 3 arrays diferentes.
const randomGame = () => {
    for (let i = 0; i < questionsOne.length; i++) {
        //Este bucle se recorre 27 veces generando un número (0,1,2) 27 veces y añadiéndolo la pregunta correspondiente al rosco.
...
const checkWord = (e) => {
    ...
    if (pressedKey === 'Enter') {
        if (usedWord.toLowerCase() === '') {
            ...
            //Si es igual, el fondo del círculo de la letra se pinta en verde.
            lettersHTML[currentPosition].style.backgroundColor = '#77dd77';
            //Resetea el valor de la caja (la pone vacía).
            answerHTML.value = '';
            answerHTML.placeholder = 'Escribe tu respuesta';
            //Cambia el estatus de questions a 1 (al ser correcta).
            questions[currentPosition].status = 1;
            lettersHTML[currentPosition].style.boxShadow = 'none';
            //Función que actualiza currentPosition.
            nextPosition();
            //Pone la sombra en la siguiente letra.
            lettersHTML[currentPosition].style.boxShadow = '0 0 15px 3px #fff';
        } else {
            //Si la palabra insertada no es correcta.
        ...
        //Aquí actualiza la pregunta, mostrando la siguiente.
        questionHTML.textContent = questions[currentPosition].question;
        //Si no es un enter y es un espacio.
    ...
    //Cuando se acaba el juego, todas las letras son verdes y/o rojas.
    if (isPlaying === false) {
        lettersHTML[currentPosition].style.boxShadow = 'none';
    }
};
...
const nextPosition = () => {
    do {
        ...
        //Ejecutame constantemente lo que haya dentro del do, hasta que encuentre 0 (donde pueda parar) y esté jugando.
    } while (questions[currentPosition].status !== 0 && checkNotFinish());
};

//Recorre el array de preguntas, comprobando su estatus.
const checkNotFinish = () => {
    ...
    //Si todos los estatus son distintos de 0, finaliza el juego y pasamos al bloque 3.
    isPlaying = false;
    document.getElementById('two').style.display = 'none';
    document.getElementById('two').style.visibility = 'hidden';
    document.getElementById('three').style.display = 'block';
...
//Recorre todas las preguntas con el estatus 1 y los suma.
const userScore = () => {
...
//Añade y coloca la puntuación y el usuario en el ranking.
const addScore = () => {
    for (let i = 0; i < scoreRanking.length; i++) {
        //Reccore cada una de las puntuaciones del ranking hasta que encuentra una menor que la puntuación del jugador.
        if (scoreRanking[i].points < finalScore) {
            //Añade la puntuación en la posición correcta gracias a splice.
            scoreRanking.splice(i, 0, { name: userName, points: finalScore });
            return;
        }
    }
    //Si no ha encontrado ninguna puntuación menor, la añadirá al final.
    scoreRanking.push({ name: userName, points: finalScore });
};

//Esta función carga las puntuaciones (calcula, añade y muestra).
const loadScores = () => {
    userScore();
    addScore();
    //Aquí se pinta la tabla del ranking.
